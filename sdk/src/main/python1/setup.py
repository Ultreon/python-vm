    ...
    def compile(p: str):
        ...
        for file in os.listdir(os.path.join(sys.argv[1], path)):
            if file.endswith(".py"):
                ...
                if os.system(f'{sys.executable} "{os.environ["PYVM_PARSER"]}" -o "{sys.argv[2]}" -p "{sys.argv[3]}.{p.replace("/", ".")}" "{p}/{file}"') != 0:
                    # exit(1)
                    continue
                a = f'java -cp {str(PYVM_COMPILER_PATH).replace("\\", "/")}{os.pathsep}{os.environ.get("PYVM_CLASSPATH").replace("\\", "/")} dev.ultreon.pyvm.compiler.AstCompiler {sys.argv[2].replace("\\", "/")}/{sys.argv[3].replace(".", "/")}/{p}/{file[:file.rfind(".")]}.ast.json "{sys.argv[3].replace("\\", "/").replace("/", ".")}.{p.replace("/", ".")}.{file[:file.rfind(".")]}" {sys.argv[2].replace("\\", "/")}/classes/'
