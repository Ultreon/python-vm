...
public class PyInheritorClass implements JvmClass, PyCompileClass {
    ...
    private final Type[] dynamicSuperTypes;
    private JvmClass[] dynamicSuperClasses;

    public PyInheritorClass(Path path, String name, int columnNo, Location location, Type... superClasses) {
        this.name = name + "$";
        ...
        this.owner = Type.getObjectType(owner.getInternalName() + "$");
    ...
    public void expectReturnType(PythonCompiler compiler, JvmClass returnType, Location location) {
        if (!doesInherit(compiler, returnType.type(compiler))) {
            throw new CompilerException("Expected " + returnType.className() + " but got " + returnType.className(), location);
        }
    }
    ...
    public JvmFunction requireFunction(PythonCompiler pythonCompiler, String name, Type[] types) {
        ...
        if (function == null) {
            PythonCompiler.expectations.expectFunction(pythonCompiler, this, name, types, false, false, location);
        }
        return function;
    }
